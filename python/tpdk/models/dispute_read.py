...
class DisputeRead(BaseModel):
    ...
    events: Optional[List[WorkflowEventRead]] = None
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CREATED', 'SUBMITTED', 'OPENED', 'ABANDONED', 'OBJECTED', 'PENDING_SHIPMENT', 'SHIPPED', 'IN_TRANSIT', 'RETURNED', 'DISMISSED', 'RESOLVED', 'PENDING']):
    ...
    def platform_actor_type_validate_enum(cls, value):
        ...
        if value not in set(['RULING', 'AI', 'CUSTOMER_CARE', 'PLATFORM_SUPPORT', 'null']):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "ulid": obj.get("ulid"),
            "transaction": TransactionRead.from_dict(obj["transaction"]) if obj.get("transaction") is not None else None,
            ...
            "parcels": [ParcelRead.from_dict(_item) for _item in obj["parcels"]] if obj.get("parcels") is not None else None,
            "views": [ViewRead.from_dict(_item) for _item in obj["views"]] if obj.get("views") is not None else None,
            "metadata": [MetadataRead.from_dict(_item) for _item in obj["metadata"]] if obj.get("metadata") is not None else None,
            "events": [WorkflowEventRead.from_dict(_item) for _item in obj["events"]] if obj.get("events") is not None else None,
